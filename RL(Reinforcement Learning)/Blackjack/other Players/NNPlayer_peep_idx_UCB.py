...
class PlayerMLP:
    def __init__(self, casino):
        ...
        #self.optimizer = optim.Adamax(self.qf.parameters(), lr=0.002, weight_decay=1e-4)
        ...
        self.lr_scheduler = optim.lr_scheduler.MultiStepLR(self.optimizer, milestone, gamma=1/np.sqrt(5))
        #self.lr_scheduler = optim.lr_scheduler.LambdaLR(self.optimizer, lr_lambda = lambda epoch: 1/math.sqrt((epoch+1)/10000))
        # UCB 의 경우 MultiStepLR 이 훨씬 좋음

        self.batch_size = 1
        self.n_idx = 4
        self.n = [0] * self.n_idx

        #self.NV = torch.ones([12,23,2,4,4]) #state-action space for counting visiting
        self.NV = np.ones([12,23,2,self.n_idx,4],dtype=np.int32)
        self.reward_shaping = torch.zeros(self.NV.shape)
    ...
    def reward_initialize(self):
        ...
        for i in range(2,12):
            for j in range(4):
                self.reward_shaping[i][11][0][j][2] = weight
        for i in range(2,10):
            for j in range(4):
                self.reward_shaping[i][10][0][j][2] = weight
        for i in range(3,7):
            for j in range(4):
                self.reward_shaping[i][9][0][j][2] = weight

        for j in range(4):
            self.reward_shaping[6][19][1][j][2] = weight
        for i in range(2,7):
            for j in range(4):
                self.reward_shaping[i][18][1][j][2] = weight
        for i in range(3,7):
            for j in range(4):
                self.reward_shaping[i][17][1][j][2] = weight
        for i in range(4,7):
            for j in range(4):
                for z in range(15,17):
                    self.reward_shaping[i][z][1][j][2] = weight
        for i in range(5,7):
            for j in range(4):
    ...
    def Batch_update(self,batch_size):
        ...
        for i in range(len(sample)):
            ...
            if r > 1:
                ...
                #r = r * (1+(1/self.n_episode))
            #r = r * (1 + self.reward_shaping[dx,ps,pu,p,aa])
