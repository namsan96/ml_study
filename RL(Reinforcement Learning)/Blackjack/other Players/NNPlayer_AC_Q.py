...
class PlayerMLP:
    def __init__(self, casino):
        ...
        self.pocket = 0
        self.make_net()
        #self.optimizer = optim.SGD(self.qf.parameters(), lr=0.001, momentum = 0.9, weight_decay = 0.1) # Add weight decay if you want regularization.
        self.optimizerqf = optim.Adam(self.qf.parameters(), lr=0.001, weight_decay=0)
        self.optimizerpf = optim.Adam(self.pf.parameters(), lr=0.0001, weight_decay=0)

        milestone = [10000, 50000, 100000]
        self.lr_schedulerqf = optim.lr_scheduler.MultiStepLR(self.optimizerqf, milestone, gamma=1/np.sqrt(10))
        self.lr_schedulerpf = optim.lr_scheduler.MultiStepLR(self.optimizerpf, milestone, gamma=1/np.sqrt(10))
        #self.lr_schedulerqf = optim.lr_scheduler.LambdaLR(self.optimizerqf, lr_lambda = lambda epoch: 1/math.sqrt((epoch+1)/1000))
    ...
    def make_net(self):
        H = 64
        lsizeqf = [self.dimS(), H, H, H, self.nA]
        lsizepf = [self.dimS(), H,H,H, self.nA]
        self.qf = Q_Net(lsizeqf)
    ...
    def TD_update_Q(self, s,a,r,sp):
        ...
        self.optimizerqf.zero_grad()
        loss.backward()
        self.optimizerqf.step()
